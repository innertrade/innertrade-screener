...
def build_markup(sym: str):
    ...
    return {"inline_keyboard": [[
        ...
        {"text": "↗️ TradingView",   "url": url_tv},
    ]]}

def fetch_oi_z(sym: str):
    """Вернёт z-score OI по окну; None при ошибке/мало данных."""
    try:
        r = s.get(
...
def arrows(z, volx, oi_z):
    """Вернёт стрелки ↑/≈0/↓ для цены, объёма, OI."""
    def arrow(val, up_th, dn_th=None):
        ...
        return "≈0"
    a_p  = arrow(z,    Z_TH)
    a_v  = "↑" if volx >= V_TH else "≈0"
    a_oi = "↑" if (oi_z is None and os.getenv("FORWARD_ALLOW_OI_NONE","false").lower()=="true") else (arrow(oi_z, OI_TH) if oi_z is not None else "≈0")
    return a_p, a_v, a_oi

def classify(a_p, a_v, a_oi):
    """Только 2 сценария: LONG / SHORT. Остальное — None (не шлём)."""
    # LONG: ↑ ↑ ↑
    if a_p=="↑" and a_v=="↑" and a_oi=="↑":
        return "LONG", "🟢📈"
    # SHORT: ↓ ↑ ↑
    if a_p=="↓" and a_v=="↑" and a_oi=="↑":
        return "SHORT","🔴📉"
        ...
        try:
            ...
            for sig in j.get("data", []):
                ...
                text = (
                    f"{mark} {sym}\n"
                    f"Цена {px}\n"
                    f"{legend} | z={z:+.2f} | vol×{vol:.2f} | OI z={('—' if oi_z is None else f'{oi_z:+.2f}')}\n"
                    f"Оборот 24ч ≈ {fmt_money(v24)}"
                )
