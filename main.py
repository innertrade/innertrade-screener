# main.py — v1.8.5-psycopg-fix
# Fix: все SQL-плейсхолдеры переведены на %s для psycopg3.
...
if not WEBHOOK_BASE or not WEBHOOK_BASE.startswith("http"): raise RuntimeError("WEBHOOK_BASE is required (https://...)")
if not WEBHOOK_SECRET: raise RuntimeError("WEBHOOK_SECRET is required")
            ...
            async with rest_session.get(url, params=params) as resp:
                ...
                if 200 <= status < 300:
                    try: return json.loads(text)
                        ...
                        log.warning("REST %s json parse error [%s] %s", tag, status, url); return None
                else:
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if (data.get("topic") or "").startswith("tickers."):
                        ...
                        for it in items:
                            ...
                            try: last = float(it.get("lastPrice", "0") or 0)
                            except: last = 0.0
                            try: p24 = float(it.get("price24hPcnt", "0") or 0) * 100.0
                            except: p24 = 0.0
                            try: turnover = float(it.get("turnover24h", "0") or 0)
                            except: turnover = 0.0
                            ws_cache[symbol] = {"last": last, "p24": p24, "tov": turnover, "ts": ws_last_msg_iso}
                            await upsert_ticker(symbol, last, p24, turnover)
                elif msg.type == aiohttp.WSMsgType.ERROR:
                    log.error("WS error: %s", msg.data); break
                elif msg.type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.CLOSE):
                    log.warning("WS closed by server"); break
    except Exception as e:
...
async def poll_oi_loop():
    if not ENABLE_OI_POLL: return
    log.info("OI polling enabled: every %ss, interval=%smin", OI_POLL_SECS, OI_INTERVAL_MIN)
    while True:
        t0 = asyncio.get_event_loop().time()
        try:
            for s in SYMBOLS:
                data = await rest_get_json("/v5/market/open-interest", {"category":"linear","symbol":s,"interval":str(OI_INTERVAL_MIN)})
            ...
            log.info("OI poll cycle done in %.2fs", asyncio.get_event_loop().time() - t0)
            await asyncio.sleep(OI_POLL_SECS)

async def poll_price_loop():
    if not ENABLE_PRICE_POLL: return
    log.info("Price polling enabled: every %ss (1h candles ~%sh back)", PRICE_POLL_SECS, PRICE_LOOKBACK_H)
    while True:
        try:
            for s in SYMBOLS:
                data = await rest_get_json("/v5/market/kline", {"category":"linear","symbol":s,"interval":"60","limit":str(PRICE_LOOKBACK_H)})
                if not data:
                    log.warning("Kline %s fetch failed (both)", s); continue
                if data.get("retCode") != 0:
                    log.warning("Kline %s retCode=%s msg=%s", s, data.get("retCode"), data.get("retMsg")); continue
                try:
...
async def cmd_activity2(message: Message):
    ...
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
            for i,(sym,p24,tov,last) in enumerate(rows,1):
                await cur.execute("SELECT COALESCE(SUM(count),0), COALESCE(SUM(qty_sum),0) FROM trades_1m WHERE symbol=%s AND ts>%s", (sym, since))
                cnt, qty = await cur.fetchone()
                await cur.execute("SELECT bid,ask,bq,aq,spread_bps,depth_usd FROM ob_1m WHERE symbol=%s ORDER BY ts DESC LIMIT %s", (sym, 1))
                ob = await cur.fetchone()
                if ob: bid,ask,bq,aq,spread_bps,depth_usd = ob
                else: bid,ask,bq,aq,spread_bps,depth_usd = (None,)*6
                await cur.execute("SELECT oi_usd FROM oi_1m WHERE symbol=%s AND ts>%s ORDER BY ts ASC LIMIT %s", (sym, since, 1))
                ...
                score = 0.0
                if tov and tov > 0: score += min(1.0, (tov/1e9))
                if cnt and cnt > 0: score += min(1.0, (cnt/4e5))
                if depth_usd and depth_usd > 0: score += min(0.5, depth_usd/1e6 * 0.5)
                if spread_bps is not None: score += 0.2 if spread_bps <= 0.02 else (0.1 if spread_bps <= 0.05 else 0.0)
                if oi_delta_pct is not None: score += max(-0.5, min(0.5, oi_delta_pct/20.0))
...
async def cmd_passport(message: Message):
    ...
    lines = [f"{sym}"]
    if last is not None: lines.append(f"last: {last}")
    if p24 is not None:  lines.append(f"24h% (WS): {p24:.4f}")
    if tov is not None:  lines.append(f"turnover24h: {tov:.2f}")
    if d24 is not None:  lines.append(f"Δ24h (1h kline): {d24:+.2f}%")
    if d7 is not None:   lines.append(f"Δ7d (1h kline): {d7:+.2f}%")
    if upd is not None:  lines.append(f"updated_at: {upd}")
    await message.answer("\n".join(lines))
...
async def cmd_now(message: Message):
    ...
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
            ...
            w = await cur.fetchone()
    if not w:
        await message.answer(f"{sym}: нет данных в ws_ticker."); return
    last, p24, tov, upd = w
...
async def cmd_diag_trades(message: Message):
    ...
    sym = parts[1].upper(); n = int(parts[2]) if len(parts) > 2 else 10
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT ts, count, qty_sum FROM trades_1m WHERE symbol=%s ORDER BY ts DESC LIMIT %s", (sym, n))
            rows = await cur.fetchall()
    if not rows: await message.answer(f"trades_1m {sym}: нет строк"); return
    lines = [f"trades_1m {sym} (latest {n})"]
    for ts,c,q in rows: lines.append(f"{ts.isoformat()}  count={c}  qty_sum={q}")
    await message.answer("\n".join(lines))
...
async def cmd_diag_ob(message: Message):
    ...
    sym = parts[1].upper(); n = int(parts[2]) if len(parts) > 2 else 5
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute("SELECT ts,bid,ask,bq,aq,spread_bps,depth_usd FROM ob_1m WHERE symbol=%s ORDER BY ts DESC LIMIT %s", (sym, n))
            rows = await cur.fetchall()
    if not rows: await message.answer(f"ob_1m {sym}: нет строк"); return
    lines = [f"ob_1m {sym} (latest {n})"]
    for ts,bid,ask,bq,aq,sp,dep in rows:
        lines.append(f"{ts.isoformat()}  bid={bid} ask={ask}  bq={bq} aq={aq}  spread={sp:.2f}bps  depth≈{int(dep):,}".replace(","," "))
    await message.answer("\n".join(lines))
...
async def cmd_diag_oi(message: Message):
    ...
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
            ...
            rows = await cur.fetchall()
    if not rows: await message.answer(f"oi_1m {sym}: нет строк"); return
    lines = [f"oi_1m {sym} (latest {n})"]
    for ts,oi in rows: lines.append(f"{ts.isoformat()}  oi≈${int(oi):,}".replace(","," "))
    await message.answer("\n".join(lines))
...
async def on_text(message: Message):
    ...
    elif t == "📄 Паспорт": await message.answer("Usage: /passport SYMBOL\nНапр.: /passport BTCUSDT")
    else:
        await message.answer("Принял: {t}\n(команды: /start /status /activity /activity2 /volatility /trend /diag_trades /diag_ob /diag_oi /passport /now)".format(t=t))
...
async def on_startup():
    ...
    # REST pollers
    if ENABLE_OI_POLL: oi_task = asyncio.create_task(poll_oi_loop())
    if ENABLE_PRICE_POLL: price_task = asyncio.create_task(poll_price_loop())
    # webhook
    assert bot
...
async def on_shutdown():
    global ws_task, oi_task, price_task, ws_session, rest_session, db_pool, bot
    if bot:
        with contextlib.suppress(Exception): await bot.delete_webhook(drop_pending_updates=False)
    ...
    if ws_session: with contextlib.suppress(Exception): await ws_session.close()
    if rest_session: with contextlib.suppress(Exception): await rest_session.close()
    if db_pool: with contextlib.suppress(Exception): await db_pool.close()
